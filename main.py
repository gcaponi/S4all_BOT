...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Costruisci il testo completo delle FAQ
    help_text = "🗒️Ciao! Per favore prima di fare qualsiasi domanda o ordinare leggi interamente il listino dopo la lista prodotti dove troverai risposta alla maggior parte delle tue domande: tempi di spedizione, metodi di pagamento come ordinare ecc. 🗒️\n\n"
    help_text += "📍NOTA BENE: la qualità è la priorità principale, i vari brand sono selezionati direttamente tra i migliori sul mercato, se cerchi prodotti scadenti ed economici non acquistare qui!\n\n"
    help_text += "🔴🔴Se vuoi puoi lasciarmi la tua Email per essere avvertito in caso di cambio contatto Telegram {tra qualche mese mi sposto su una nuova piattaforma per la sicurezza di tutti} e per essere avvertito all' arrivo dei prodotti terminati o prodotti nuovi e promozioni🔴🔴\n\n\n"
    
    # Aggiungi tutte le FAQ con domanda e risposta
    for item in faq_list:
        help_text += f"## {item['domanda']}\n{item['risposta']}\n\n"
    
    # Messaggio troppo lungo? Dividi in più messaggi
    if len(help_text) > 4096:
        ...
        for line in lines:
            if len(current_part) + len(line) + 1 > 4000:  # Lascia margine
...
PAYMENT_KEYWORDS = ["bonifico", "USDT", "crypto", "XMR", "BTC", "ETH", "USDC"]
