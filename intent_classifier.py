    ...
    def _classify_by_simple_rules(self, message, debug=False):
        ...
        if any(verb in message for verb in self.wish_verbs):
            if has_product or has_category:
                ...
                # PATCH: Se chiede di ordinare senza specificare cosa → FAQ
                if any(word in message for word in ['ordinare', 'ordine', 'ordina', 'ordino']):
                    return "faq", 0.90  # "vorrei ordinare", "come si ordina?"
